...
class VAR(nn.Module):
    ...
    def autoregressive_infer_cfg(
        ...
        cfg_label_B = torch.cat([label_B, unc_label_B])  
        
        sos = cond_BD = self.transformer.class_embedding(cfg_label_B)  
        ...
        ) + self.transformer.position_embedding.expand(2*B, -1, -1) 
        ...
        for si, pn in enumerate(self.patch_nums):
            ...
            logits_BlV = self.transformer.head(x, cond_BD)
            ...
            if not more_smooth:
                ...
                h_BChw = self.vqvae.quantize.embedding(idx_Bl) 
            else:
            ...
            h_BChw = h_BChw.transpose(1, 2).reshape(B, Cvae, pn, pn)
            ...
            if si != len(self.patch_nums) - 1:
                next_token_map_pre = next_token_map_pre.view(B, Cvae, -1).transpose(1, 2)  
                next_token_map = self.transformer.word_embed(next_token_map_pre) + \
                                lvl_pos[:B, cur_L:cur_L + self.patch_nums[si+1]**2]
                next_token_map = next_token_map.repeat(2, 1, 1) 
        
        generated_images = self.vqvae.fhat_to_img(f_hat)  
        generated_images = generated_images.add_(1).mul_(0.5)
