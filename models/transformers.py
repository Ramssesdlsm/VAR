...
class AdaptiveLayerNorm(nn.Module):
    ...
    def forward(
        ...
        gate_attn, gate_ffn, shift_attn, scale_attn, shift_ffn, scale_ffn = (
...
class AdaptiveLayerNormBeforeHead(nn.Module):
    def __init__(self, dim: int, cond_dim: int, vocab_size: int):
        ...
        self.norm = nn.LayerNorm(dim, elementwise_affine=False)
        self.modulation = nn.Sequential(
            nn.SiLU(), nn.Linear(cond_dim, 2 * dim, bias=True)
        )
...
class VARTransformer(nn.Module):
    ...
    def forward(
        ...
        assert total_seq_len <= self.max_seq_len, (
            f"Sequence length {total_seq_len} exceeds model capacity {self.max_seq_len}"
        )

        class_cond = self.class_embedding(class_labels)

        sos = class_cond.unsqueeze(1).expand(-1, self.first_scale_tokens, -1)
        sos_level_indices = torch.zeros(
            self.first_scale_tokens, dtype=torch.long, device=class_cond.device
        )
        sos = (
            sos
            + self.position_embedding[:, : self.first_scale_tokens, :]
            + self.level_embedding(sos_level_indices)
        )

        teacher_embeddings = self.word_embed(x_BLCv_wo_first_l.float())

        position_embeddings = self.position_embedding[
            :, self.first_scale_tokens : total_seq_len, :
        ]
        level_embeddings = self.level_embedding(
            level_indices[:, self.first_scale_tokens :]
        )
